...
async function getMovies (page = 1) {
  return Promise
    ...
    .then(function (resp) {
      ...
      return $('[data-testid="filter-results"], .c-finderProductCard').toArray()
    })
    .map(function (movie) {
      const $movie = cheerio.load(movie)

      return {
        title: $movie('[data-testid="product-title"], .c-finderProductCard_title').first().text().trim(),
        score: _.parseInt($movie('.c-siteReviewScore').first().text().trim())
      }
    })
    .then(function (movies) {
      return _.reject(movies, function (movie) {
        return movie.title.length === 0 || _.isNaN(movie.score)
      })
    })
}
